...
def load_model(model_path: str) -> YOLO:
    """Load and return a YOLO model from specified path."""
    return YOLO(model_path)

def predict(model: YOLO, img: np.ndarray, conf_thres: float) -> list:
    """
    Perform prediction on a single image frame using a loaded YOLO model.
    
    Args:
        model (YOLO): Loaded YOLO model
        img (np.ndarray): Input image frame (BGR format)
    ...
    # Perform prediction
    results = model(img_rgb, conf=conf_thres, verbose=False)
